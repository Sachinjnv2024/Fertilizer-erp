...
const nav = [
  ['Dashboard', LayoutDashboard], ['Sales / Billing', Receipt], ['Inventory', Package],
  ['Customers', Users], ['Reports', BarChart3], ['Settings', Settings]
]
...
export default function App() {
  ...
  async function loadData() {
    ...
    setProducts(p || []); setStock(s || []); setSuppliers(sp || []); setCustomers(sp2 || []); setPayments(pay || []); setExpenses(exp || []); if(bs) setBusinessSettings(bs); setLoading(false)
  }
    ...
    <main className="main">
      ...
      <section className="content">
        <div className="referenceNav">
          {nav.map(([label,Icon])=><button key={label} className={active===label?'active':''} onClick={()=>setActive(label)}><Icon size={14}/> {label==='Sales / Billing'?'New Sales Invoice':label==='Customers'?'Dealer & Farmer':label}</button>)}
          {['Purchase','Payments','Returns'].map(label=><button key={label} className={active===label?'active':''} onClick={()=>setActive(label)}>{label==='Purchase'?'Purchase / Stock Intake':label==='Payments'?'Receipt / Payment':'Returns'}</button>)}
        </div>
        {active==='Dashboard' && <div className="referenceQuick"><button onClick={()=>setShowSale(true)}><b>🧾</b>Sale</button><button onClick={()=>setShowCustomer(true)}><b>👨‍🌾</b>Farmer</button><button onClick={()=>setActive('Suppliers')}><b>🏪</b>Dealer</button><button onClick={()=>setActive('Inventory')}><b>📦</b>Stock</button></div>}
        ...
        {active==='Sales / Billing' && <Sales products={products} customers={customers} stock={stock} reload={loadData} businessSettings={businessSettings}/>}
        ...
        {active==='Settings' && <SettingsView products={products} stock={stock} suppliers={suppliers} customers={customers} payments={payments} expenses={expenses} businessSettings={businessSettings} onBusinessSettingsSaved={setBusinessSettings}/>}
      ...
      <nav className="bottomReferenceNav">{[
        ['Dashboard','⌂','Home'],['Sales / Billing','🧾','Billing'],['Inventory','📦','Stock'],['Customers','👥','Parties'],['Reports','📊','Reports'],['Settings','☰','More']
      ].map(([key,icon,label])=><button key={key} className={active===key?'active':''} onClick={()=>setActive(key)}><span className="navIcon">{icon}</span>{label}</button>)}</nav>
    </main>
...
function Sales({products,customers,stock,reload,businessSettings}) {
...
function InvoicePrint({sale,businessSettings,close}) {
...
function SettingsView({products=[],stock=[],suppliers=[],customers=[],payments=[],expenses=[],businessSettings,onBusinessSettingsSaved}) {
  const [form,setForm]=useState(businessSettings||{id:1,business_name:'My Fertilizer Store',address:'',mobile:'',email:'',gstin:'',state:'',invoice_prefix:'INV'})
  const [saving,setSaving]=useState(false)
